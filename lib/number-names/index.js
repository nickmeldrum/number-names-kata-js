 ...
 * @returns {string}
 */
const buildNumberWithUnits = numArray => {
  let word = ''
 ...
 * @returns {string}
 */
const getUnit = (numArray, currentIndex) => {
...
/**
 * Builds the three value chunk
 * @param numArray
 * @returns {string}
 */
const buildNumber = numArray => {
  let word = ''

  // add hundreds value if exists
  if (numArray.length === 3 && Number(numArray[0]) !== 0) {
    word += `${ones[Number(numArray[0]) - 1]} hundred`
  }

  // check if teen number
  if (!isTeenNumber(numArray)) {
    ...
    // if teen number get number from teens array
